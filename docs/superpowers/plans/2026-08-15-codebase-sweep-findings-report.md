...
| `apps/web/apply-indexes.js` | **Still genuinely uncertain (2nd audit, 2026-08-15).** No package.json script, README, or doc references it as an intentional manual tool. Traces to the original 2026-05-05 monorepo-migration commit, untouched since. Git history/docs can't rule out ad-hoc manual use — resolvable only by asking directly whether this is still run by hand. |
| `apps/web/clean-pending-owner.js` | Same as above — no doc/script reference found, untouched since the 2026-05-05 migration commit, still genuinely uncertain pending a direct answer on manual-use habits. |
...
| `apps/web/encode-password.js` | Same uncertainty class as `apply-indexes.js`/`clean-pending-owner.js` — no doc/script reference, untouched since 2026-05-05, still open pending direct confirmation. |
| `apps/web/scripts/generate-favicons.ts` | Same open question — no doc reference found either way. |
| `apps/web/scripts/list-users.ts` | Traces to the 2026-05-29 Prisma v7 upgrade commit, no doc reference found since. Still open. |
| `apps/web/scripts/make-admin.ts` | Same commit lineage as `list-users.ts`, same open question — these two in particular read like plausible ongoing admin tools, worth asking about directly. |
...
| `scripts/backfill-venue-types.ts` | **CORRECTED (2nd audit, 2026-08-15):** the original "see prior backfill scripts... for precedent" claim is false — this is the *only* backfill script in the repo, no precedent exists. It's self-documented in its own header as a "one-shot script" tied to the shipped `venueType` field (commit from 2026-06-28). Whether it's safe to delete depends on whether it already ran against prod — the sandbox couldn't query the prod DB directly to confirm `venueType IS NOT NULL` coverage. Resolvable via a direct `psql` check against prod, not by more code reading. |

**Judgment note:** the root `*.js`/`scripts/*` one-off files split into two groups after the second audit pass: `dashboard-analytics.tsx`/`manage-staff-role-dialog.tsx` are now confirmed safe to delete (the "planned-but-unwired" theory didn't hold up), while the ops scripts (`apply-indexes.js`, `clean-pending-owner.js`, `encode-password.js`, `generate-favicons.ts`, `list-users.ts`, `make-admin.ts`) remain genuinely open — no amount of further code/doc reading resolves whether these are still run by hand, since manual CLI usage leaves no trace in the repo. **This needs a direct answer from the user before Stage 2 touches any of these 6 files**, not another automated pass. `backfill-venue-types.ts` similarly needs a live prod DB check, not more reading, to confirm the backfill already ran.
...
**Still genuinely open — cannot be resolved by more code reading:**
- **6 ops scripts** (`apply-indexes.js`, `clean-pending-owner.js`, `encode-password.js`, `scripts/generate-favicons.ts`, `scripts/list-users.ts`, `scripts/make-admin.ts`): no doc/script/README reference exists either way, and manual CLI usage (`node script.js`, `npx tsx script.ts`) leaves no trace in git history. **This needs a direct answer from the user on whether any of these are still run by hand** — `make-admin.ts`/`list-users.ts` in particular read like plausible ongoing admin tools worth asking about specifically, not just assuming dead.
- **`scripts/backfill-venue-types.ts`**: self-documented as a one-shot migration script tied to the `venueType` field (shipped 2026-06-28). Whether it's safe to delete depends on whether it already ran against prod — resolvable via a direct `psql` check for `venueType IS NOT NULL` coverage, not by more source reading. The original report's justification ("see prior backfill scripts for precedent") was itself wrong — this is the only backfill script in the repo, no precedent to check.
