...
1. `apps/web/app/api/user/profile/route.ts:10-13` — `displayName` is destructured from the raw body and checked only for truthiness/type/non-empty-after-trim (`if (!displayName || typeof displayName !== "string" || displayName.trim().length < 1)`). **No upper bound at all** — a user could set an arbitrarily long `displayName`, which is then written to `User.displayName` and rendered throughout the app (staff lists, patron-log attribution, feedback admin view, Discord embeds via `formatFeedbackSubmittedEmbed`'s `safeUserName` truncation — that one's already capped at the Discord-embed layer, but the raw DB value and every _other_ render site, e.g. the staff table, is not).
