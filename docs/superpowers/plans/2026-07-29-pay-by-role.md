...
<div className="flex flex-col items-end gap-1 shrink-0 ml-2">
  ...
  {role.hourlyRate && <Badge variant="outline">{Number(role.hourlyRate).toLocaleString()}/hr</Badge>}
</div>
```
...
const effectiveRate = baseRate !== undefined && baseRate !== null ? new Decimal(baseRate) : staffMembership.hourlyRate
...
const shiftDetails = eligibleShifts.map((shift) => {
  let hours = 0
  if (shift.actualStart && shift.actualEnd) {
    hours = Math.round(((shift.actualEnd.getTime() - shift.actualStart.getTime()) / (1000 * 60 * 60)) * 100) / 100
    totalHours += hours
  }
  return {
    id: shift.id,
