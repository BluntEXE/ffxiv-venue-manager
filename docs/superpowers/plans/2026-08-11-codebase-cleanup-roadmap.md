...
- **Bug, not cleanup — found 2026-08-11 while creating a test event:** `app/dashboard/[slug]/events/new/page.tsx:105-106` builds the event start/end using `new Date(year, month, day, hours, minutes)` with no UTC marker, so the picker input is silently interpreted as the *operator's own browser-local timezone* and converted to UTC from there — unlike `create-shift-dialog.tsx`, which explicitly forces Server Time via a literal `Z` suffix (`${date}T${startTime}:00Z`), and unlike the app's stated convention that all schedule input is Server Time. No label on the form tells the operator which one it is. Symptom: an operator typing a time meaning "now, in Server Time" gets a stored UTC instant offset by their own timezone, and the event doesn't show as live when it should. Fix: apply the same `${date}T${startTime}:00Z`-style literal-UTC construction used in the shift dialog, and add the same "All times are in Server Time (ST)" hint text near the pickers.
