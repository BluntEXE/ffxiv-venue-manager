...
const MUSIC_KEYWORD =
  /(?:music|soundscape|sounds?track|set|dj\s*line[-\s]?up)\s*(?:curated\s+)?by[ \t]*:?[ \t]*\n?([^│|\n]{3,200})/i

// Time tokens
const TIME_TOKEN_SRC = "\\d{1,2}(?::\\d{2})?\\s*(?:am|pm)?"
// Slot range: supports - – and → as separators between start/end times
const SLOT_RE = new RegExp(`(${TIME_TOKEN_SRC})\\s*[-–→⟶]\\s*(${TIME_TOKEN_SRC})\\s*(?:st|server\\s*time)`, "gi")
const TIME_RANGE_ST = new RegExp(`(${TIME_TOKEN_SRC})\\s*[-–→⟶]\\s*(${TIME_TOKEN_SRC})\\s*(?:st|server\\s*time)`, "i")
const TIME_SINGLE_ST = new RegExp(`(${TIME_TOKEN_SRC})\\s*(?:st|server\\s*time)`, "i")

// Inline: "5-7 ST: Name" or "5-7 ST → Name"
const INLINE_SLOT_RE =
  /\d{1,2}(?::\d{2})?(?:\s*(?:am|pm))?\s*[-–→⟶]\s*\d{1,2}(?::\d{2})?(?:\s*(?:am|pm))?\s*(?:st|server\s*time)\s*[:\-→⟶]\s*([^│|\n,]{2,50})/gi

// "6 PM ST – 8 PM ST → DJ Name" - ST embedded between each time, arrow before name
const RANGE_WITH_ST_ARROW =
  /\d{1,2}(?::\d{2})?\s*(?:am|pm)?\s*(?:st|gmt[+-]\d+)\s*[-–]\s*\d{1,2}(?::\d{2})?\s*(?:am|pm)?\s*(?:st|gmt[+-]\d+)\s*[→⟶➤]\s*([^│|\n]{2,50})/gi

// "17:00 → 18:00 ST - Ti Beats" - arrow between times, dash before name
const ARROW_RANGE_NAME =
  /\d{1,2}(?::\d{2})?\s*(?:am|pm)?\s*[→⟶➤]\s*\d{1,2}(?::\d{2})?\s*(?:am|pm)?\s*(?:st|gmt[+-]\d+)?\s*[-:]\s*([^│|\n]{2,50})/gi
...
const GMT_TIME_LINE =
  /time[:\s]+(\d{1,2}(?::\d{2})?\s*(?:am|pm))\s*[-–]\s*(\d{1,2}(?::\d{2})?\s*(?:am|pm))\s*gmt([+-]\d+)/i
...
const SLOT_INLINE_ST_RE =
  /(\d{1,2}(?::\d{2})?\s*(?:am|pm)?)\s*(?:st|gmt[+-]\d+)\s*[-–]\s*(\d{1,2}(?::\d{2})?\s*(?:am|pm)?)\s*(?:st|gmt[+-]\d+)/gi
...
function extractTime(text: string): string | undefined {
  ...
  if (inlineStSlots.length >= 2)
    return `${inlineStSlots[0][1].trim()}-${inlineStSlots[inlineStSlots.length - 1][2].trim()} ST`
  ...
  const slotTimes = [...text.matchAll(/^(\d{1,2}:\d{2})\s*(?:[-–]\s*\S|\s{2,}\S)/gm)].map((m) => m[1])
...
function extractDJs(text: string): string {
  ...
  const rangeStArrow = [...text.matchAll(RANGE_WITH_ST_ARROW)].map((m) => clean(m[1])).filter(validDJ)
  if (rangeStArrow.length) return rangeStArrow.join(", ")

  // "time ST: Name" or "5-7 ST → Name" inline
  const inlineSlots = [...text.matchAll(INLINE_SLOT_RE)].map((m) => clean(m[1])).filter(validDJ)
  if (inlineSlots.length) return inlineSlots.join(", ")

  // "17:00 → 18:00 ST - Ti Beats"
  const arrowNames = [...text.matchAll(ARROW_RANGE_NAME)].map((m) => clean(m[1])).filter(validDJ)
  ...
  const singleDash = [...text.matchAll(SINGLE_TIME_NAME)].map((m) => clean(m[1])).filter(validDJ)
  if (singleDash.length) return singleDash.join(", ")

  // "HH:MM  Name" single-time with double space (no dash)
  const singleSpace = [...text.matchAll(SINGLE_TIME_SPACE_NAME)].map((m) => clean(m[1])).filter(validDJ)
