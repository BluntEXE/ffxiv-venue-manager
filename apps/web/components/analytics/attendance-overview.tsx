  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Typical patron volume by time of day (based on last 20 events)</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="h-[300px] w-full">
          <ResponsiveContainer width="100%" height="100%" minWidth={0}>
            <AreaChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                    ...
                    return (
                      <div className="rounded-lg border bg-[#0a0f1e] p-2 shadow-lg border-[rgba(0,180,255,0.25)]">
                        <div className="flex flex-col">
                          <span className="text-[0.70rem] uppercase text-muted-foreground mb-1">{label}</span>
                          <span className="font-bold text-[var(--xiv-blue)]">~{payload[0].value} Patrons</span>
                        </div>
                      </div>
                    )
                  }
              ...
              />
              <Area type="monotone" dataKey="avgCount" stroke="#00b4ff" strokeWidth={2} fill="url(#colorAvg)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </CardContent>
