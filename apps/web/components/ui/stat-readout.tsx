...
function StatReadout({
  ...
  return (
    <div className={cn("flex flex-col gap-1", className)}>
      ...
      <span className="text-[0.64rem] font-semibold uppercase tracking-[0.12em] text-[var(--fg-faint)] mt-[14px] block">
        {label}
      </span>
      <div className="font-[var(--font-outfit)] font-bold text-[1.7rem] leading-none mt-1">{value}</div>
      {(delta || subtext) && (
        <div className={cn("flex items-center gap-1 text-xs mt-0.5", deltaColor)}>
          ...
          {subtext && !delta && <span className="text-muted-foreground">{subtext}</span>}
        </div>
      )}
    </div>
  )
